RuiCoin Core version *0.15.1* is now available from:

  <https://download.ruicoin.org/ruicoin-0.15.1/>
...
A number of changes to the way RuiCoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 popos per kB (0.00001 LTC). This is to prepare for dropping the minimum transaction fee to 0.1 popos per kB (0.0001 LTC) in the next release.
